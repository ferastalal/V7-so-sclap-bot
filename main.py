...
def session_label() -> str:
    ...
    t = now.strftime("%H:%M")
    if "09:35" <= t <= "11:00":
        return "Morning Power"
    elif "15:00" <= t <= "15:55":
        return "Closing Power"
    else:
        return "Normal"
...
def market_move():
    try:
        ...
        if spy.empty or qqq.empty or len(spy) < 10:
            market_cache.update({"value": (0.0, "Market unknown"), "time": now_time})
            return market_cache["value"]
...
def real_quality_filter(move_1m, move_3m, move_5m, rel_strength,
    ...
    if move_1m > move_1m_min:
        ...
        warnings.append("Move 1m weak")
    if move_3m > move_3m_min:
        score += 1
    else:
        warnings.append("Move 3m weak")
    if move_5m > move_5m_min:
        score += 1
    else:
        warnings.append("Move 5m weak")
    if rel_strength > 0.001:
        score += 1
    else:
        warnings.append("Rel strength weak")
    ...
    if 0.001 <= atr_pct <= 0.025:
        ...
        warnings.append("ATR out of range")
    if -0.001 <= vwap_dist <= 0.012:
        score += 1
    else:
        warnings.append("VWAP dist out of range")
    if 45 <= rsi5 <= 75:
        score += 1
    else:
        warnings.append("RSI 5m out of range")
    aggressive = (
        score >= 5 and adx >= 28
...
def analyze(stock: str):
    try:
        ...
        if price > ema9_1 > ema21_1:
            score += 15
            reasons.append("Trend 1m (EMA9 > EMA21)")
        if ema9_5 > ema21_5:
            score += 10
            reasons.append("Trend 5m")
        if ema9_15 > ema21_15:
            score += 8
            reasons.append("Trend 15m")
        if price > ema50_1:
            score += 8
        ...
        if rel_vol >= 2.0:
            score += 20
            reasons.append(f"Volume {rel_vol:.2f}x")
        elif rel_vol >= 1.3:
            score += 15
            reasons.append(f"Volume {rel_vol:.2f}x")
        elif rel_vol >= 1.0:
        ...
        if rel_strength > 0.002:
            score += 15
            reasons.append("Strong vs market")
        elif rel_strength > 0.001:
            score += 8
            reasons.append("OK vs SPY/QQQ")
        if adx >= 30:
            score += 12
        ...
        if power:
            score += 10
            reasons.append(f"Power session {session_label()}")
        if golden:
            score += 15
...
def build_alert_message(r: dict) -> str:
    lv = r["levels"]
    w  = "\n".join([f"  {x}" for x in r["warnings"]]) if r["warnings"] else "None"
    ...
    momentum_line   = f"\n📈 Momentum acc." if r["momentum_acc"] else ""
    msg = f"""<b>{title}</b>
<b>{r['stock']}</b> | {r['session']}
{get_ksa_time()} KSA
Price: <b>{r['price']:.2f}</b>
{r['market_label']}
—
<b>Levels:</b>
Entry:  <b>{lv['entry']:.2f}</b>
Target: <b>{lv['target']:.2f}</b>
Stop:   <b>{lv['stop']:.2f}</b>
Support:    {lv['support']:.2f}
Resistance: {lv['resistance']:.2f}
{rr_emoji} R:R = <b>{lv['rr']:.1f}x</b>
—
<b>Quality:</b> {r['quality_label']}
Real Score: {r['real_score']}/9
Score: {r['score']}{aggressive_line}{macd_line}{momentum_line}
<b>Indicators:</b>
...
send("<b>SCALP BOT PRO - Started</b>\nWatching: VWAP | MACD Cross | Momentum")
...
while True:
    try:
        ...
        for stock in WATCHLIST:
            ...
            send(msg)
            log.info(f"ALERT SENT: {stock} | score={score} | real={real_score}/9 | {result['quality_label']}")
            ds.log_alert(
                stock=stock,
                score=score,
                real_score=real_score,
