...
def register_virtual_trade(stock, result, score, alert_type, now_ksa):
    ...
    trade = {
        ...
        "score": int(score),
        "real_score": int(result["real_score"]),
        "momentum_beast": bool(result["momentum_beast"]),
        "aggressive_momentum": bool(result["aggressive_momentum"]),
        "rsi1": round(float(result["rsi1"]), 2),
        "rsi5": round(float(result["rsi5"]), 2),
        "adx": round(float(result["adx"]), 2),
        "relative_volume": round(float(result["relative_volume"]), 3),
        "vwap_distance_pct": round(float(result["vwap_distance"] * 100), 3),
        "move_3m_pct": round(float(result["move_3m"] * 100), 3),
        "move_5m_pct": round(float(result["move_5m"] * 100), 3),
        "relative_strength_pct": round(float(result["relative_strength"] * 100), 3),
...
def build_daily_report(day):
    ...
    for t in trades[-12:]:
        ...
        pnl = t.get("pnl_pct") if t.get("pnl_pct") is not None else "غير محسوم"
        lines.append(
            f"{result_icon} {t['stock']} | {t['alert_time_ksa']} | {t['alert_type']} | "
            f"PnL: {pnl}% | Real {t['real_score']}/9 | RVOL {t['relative_volume']}x | "
            f"ADX {t['adx']} | {t.get('reason') or 'لم يصل هدف ولا وقف بعد'}"
        )

    lines.append("")
    lines.append("ملاحظة: هذه محاسبة افتراضية للتنبيهات، وليست تنفيذ حقيقي داخل المحفظة.")
    return "\n".join(lines)
...
def maybe_send_daily_report():
    ...
    if daily_report_sent_for == day:
        return

    if now.hour > DAILY_REPORT_HOUR_KSA or (now.hour == DAILY_REPORT_HOUR_KSA and now.minute >= DAILY_REPORT_MINUTE_KSA):
...
def analyze(stock):
    try:
        ...
        # Yahoo أحياناً آخر شمعة تكون غير مكتملة، لذلك نحسب RVOL على آخر شمعة مكتملة
